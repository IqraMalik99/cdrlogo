// app/logo/[slug]/page.jsx  (or pages/logo/[slug].jsx)
"use client";
...
export default function LogoDetail() {
    ...
    // ── Build enriched description with inline related-logo links ──────────
    const buildEnrichedDescription = (desc, relatedLogos) => {
        if (!desc || relatedLogos.length === 0) return { __html: desc || "" };

        // Try to inject a contextual sentence after the first paragraph / sentence
        const sentences = desc.split(/(?<=[.!?])\s+/);
        const insertAt = Math.min(2, sentences.length - 1); // after 2nd sentence

        const linkSentence = relatedLogos.slice(0, 3).map((rel, i) => {
            const name = rel.brand || rel.logoName;
            return `<a href="/logo/${rel.slug}" class="desc-inline-link">${name} Logo</a>`;
        }).join(", ");
    ...
    const canonicalUrl = `https://yourdomain.com/logo/${logo.slug}`;
    const metaTitle    = logo.metaTitle       || `${logo.logoName} Logo – Free Download (SVG, PNG, AI, CDR)`;
    const metaDesc     = logo.metaDescription || fullDesc.slice(0, 160);
    ...
    return (
        <>
            <Head>
                ...
                {logo.webpUrl && <meta property="og:image" content={logo.webpUrl} />}
            </Head>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Inline related links inside description ── */
  ...
  .desc-related-hint a.desc-inline-link {
    font-style:normal;
  }
                ...
                <div className="page">
                    ...
                    <div className={`layout${ready ? " ready" : ""}`}>
                        ...
                        <div className="left">
                            ...
                            {/* ── About card with enriched description ── */}
