...
// fall back to using the word "brand" as a placeholder subject.
...
function validateAIContent(parsed, { usedTitles = [], usedFaqQuestions = [], isTemplate = false } = {}) {
  ...
  if (Array.isArray(parsed.faq)) {
    ...
    if (parsed.faq.length < 1 || parsed.faq.length > 4) {
      violations.push(`faq has ${parsed.faq.length} entries — must be between 1 and 4`);
    }
  }
...
// ============================================================================
// generateMainDescription — same dedicated research-based pipeline as the
// bulk route. Case A (150–250 words, substantial verifiable history), Case B
// (30–80 words, brand real but thin notes), Case C (blank, no info at all).
// ============================================================================
...
- Never fabricate a domain that "looks right" (e.g. guessing brandname.com without verifying it's correct).`;
  ...
  const faqPoolForThisLogo = getFaqPoolForCategory(mainCategory, isTemplate);
...
with general format/technical questions. Only choose from THIS pool.

STEP 1 — HOW MANY QUESTIONS
Select between 1 and 4 questions — never a fixed number. Choose only as many
as you can answer with genuine, specific, brand-related information or
direct visual observation of THIS exact logo. If little is known about this
logo/brand, 1 or 2 questions is correct and preferred over padding to 4.
...
- Base category-specific answers on what is genuinely observable in THIS
  specific logo (its actual colors, shape, symbol, style) — never a generic
  answer that could apply to any logo in this category.
...
- If you cannot verify a fact needed to answer a chosen question, do not
  guess — choose a different question instead.
...
Never fabricate history, symbolism, or meaning that cannot be reasonably
inferred from the logo itself or verified research. A short, honest answer
...
// upgraded to match the bulk route (Tavily research-based description +
// category-keyed FAQ pool).
// ══════════════════════════════════════════════════════════════════════════
