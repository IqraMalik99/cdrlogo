...
async function generateAIContent({
  ...
  const forcedStyle = isTemplate ? "A" : STYLE_LETTERS[Math.floor(Math.random() * 4)];
  console.log(`  [style] Forced style for this logo: STYLE ${forcedStyle} ${isTemplate ? "(template — forced)" : ""}`);
...
website_used is the only field you may need to leave blank if UNKNOWN above.`;
  ...
  // ── Style section (varies for template vs non-template) ──────────────────
...
industry. You MUST write main_description using STYLE A (format-first) ONLY.
Do NOT reference a brand, company, country, or industry anywhere.
